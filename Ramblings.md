...
Here is what I figured eventually, the "oracle" is indeed able to mark but we need to query the "oracle" with the inputs. Where Grover's algorithm wins is the number of query one would have to make to the **"oracle"** vs a classical "oracle" that performs the same function. Many hours spent on internet search kind of points to this and this makes sense too. 
